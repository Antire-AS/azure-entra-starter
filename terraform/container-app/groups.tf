...
#
# HOW DOES A PERSON GET ACCESS?
#
#   Without groups (default):
#     Anyone in your organization can open the app URL and sign in.
#     No action needed — they already have access.
#
#   With a group:
#     1. Someone creates a security group (this Terraform can do it for you)
#     2. An admin adds people to the group (portal or CLI, see below)
#     3. Only group members can access the app
#     4. To give a new person access, add them to the group
#     5. To remove access, remove them from the group
#
# HOW TO ADD SOMEONE TO THE GROUP:
#
#   Portal:
#     Entra ID -> Groups -> <your group name> -> Members -> Add members
#     Search by name or email, select the person, click Select.
#
#   CLI:
...
#
#   Portal:
#     Entra ID -> Groups -> <your group name> -> Members
#     Select the person -> Remove
#
#   CLI:
...
#
#   CLI:
#     az ad group member list --group "Chat App Users" \
#       --query "[].{name:displayName, email:userPrincipalName}" -o table
#
# NOTE: Nested groups are NOT supported. Only direct members of the assigned
# group get access. If you have a parent group containing child groups,
# you must assign the child groups directly.
...
resource "azuread_group" "access" {
  ...
  # The person running Terraform is automatically an owner of the group,
  # so they can manage members.
  owners = [data.azuread_client_config.current.object_id]
}

# --- Assign the group to the app ---
# This tells Entra ID: "members of this group are allowed to sign in."
# The app_role_id "00000000-..." is the built-in "Default Access" role —
# it simply means "allowed to use this app" without any special permissions.
